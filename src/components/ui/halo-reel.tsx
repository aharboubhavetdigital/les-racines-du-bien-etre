...
export interface HaloReelProps
  ...
  stepDuration?: number;
  /** Hold the autoplay while a pointer rests on a card. @default false */
  pauseOnHover?: boolean;
  /** Spin the ring by dragging it. @default false */
  draggable?: boolean;
...
export function HaloReel({
  ...
  // Autoplay loop with smooth continuous animation
  React.useEffect(() => {
  ...
  return (
    <div
      ...
      aria-label={props["aria-label"] ?? "Halo Reel Carousel"}
      tabIndex={0}
      onKeyDown={onKeyDown}
      ...
      className={cn(
        "relative h-full w-full select-none overflow-hidden outline-none pointer-events-auto",
      ...
      {Array.from({ length: slots }, (_, i) => (
        <WheelCard
          ...
          onHoverChange={(hovered) => {
            if (pauseOnHover) hoverRef.current = hovered;
          }}
        />
      ))}
    </div>
...
function WheelCard({
  ...
  return (
    <motion.div
      ...
      className="absolute overflow-hidden rounded-2xl border border-white/20 shadow-2xl bg-[#1C2620]"
    >
      {item.src ? (
        <div className="relative w-full h-full group overflow-hidden">
          ...
          {item.title && (
            <div className="absolute bottom-3 left-3 right-3 text-white z-10 space-y-0.5">
              <p className="font-serif text-sm font-semibold leading-tight drop-shadow-md">
                {item.title}
              </p>
              {item.subtitle && (
        ...
        <div
          ...
          style={{
            backgroundColor: item.bgColor || '#20352B',
            color: item.textColor || '#FFFFFF',
          }}
        >
          {item.title ? (
            <span className="text-lg font-serif font-medium leading-tight">
              {item.title}
            </span>
          ) : null}
          {item.subtitle ? (
