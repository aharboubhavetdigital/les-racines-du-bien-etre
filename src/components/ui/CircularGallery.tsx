...
const DEFAULT_FONT = 'bold 28px Playfair Display, serif';
const DEFAULT_FONT_URL = 'https://fonts.googleapis.com/css2?family=Playfair+Display:wght@500;700&display=swap';
...
function createTextTexture(gl: any, text: string, font = 'bold 28px monospace', color = 'black') {
...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = '#181D1A', font = '28px sans-serif' }: any) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
class AppEngine {
  ...
  createMedias(items: CircularGalleryItem[] | undefined, bend = 1, textColor: string, borderRadius: number, font: string) {
    const defaultItems: CircularGalleryItem[] = [
      ...
      { image: `https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?auto=format&fit=crop&w=800&q=80`, text: '06. Suivi Personnalisé' }
    ];
